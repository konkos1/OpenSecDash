...
Install the exact Python version from `backend/.python-version` (currently 3.14.7) and the officially pinned `uv` 0.12.13 release before starting. This matches the validated Docker production runtime.
...
sudo -u opensecdash uv sync --python "$(cat .python-version)" --frozen --no-dev
```
