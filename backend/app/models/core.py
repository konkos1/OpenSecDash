...
class CrowdSecAsnBan(Base):
    ...
    provider_candidate_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    provider_candidate_first_ip: Mapped[str | None] = mapped_column(String(64), nullable=True)
    provider_candidate_last_event_id: Mapped[int | None] = mapped_column(nullable=True)
    provider_candidate_observations: Mapped[int] = mapped_column(default=0)
    provider_candidate_distinct_ip_seen: Mapped[bool] = mapped_column(Boolean, default=False)
...
class GeoIPCache(Base):
    ...
    asn_organization: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
