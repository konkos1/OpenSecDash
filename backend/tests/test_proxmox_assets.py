...
def test_proxmox_system_vmid_does_not_collide_with_json_assets(monkeypatch, db_session):
    monkeypatch.setattr(proxmox_assets, "ProxmoxClient", FakeClusterClient)
    db_session.add(System(vmid="104", hostname="old-json-host", system_type="lxc", source_plugin="json_assets", external_id="json_assets:system:104"))
    db_session.commit()

    result = sync_proxmox_assets(db_session, api_url="https://pve.local:8006", token_id="id", token_secret="secret")

    assert result["systems_created"] == 2
    assert db_session.query(System).filter(System.vmid == "104").one().source_plugin == "json_assets"
