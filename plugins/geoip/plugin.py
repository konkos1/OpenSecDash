...
class Plugin(PeriodicPlugin):
    metadata = PluginMetadata(
        ...
        description="Adds country codes to public IP events using a cached GeoIP provider lookup.",
    )
    settings = [
    ...
    locales = {
        "en": {
            "geoip.settings.enabled": "GeoIP enrichment enabled",
            "geoip.settings.enabled.help": "Adds a country code to new public-IP events when the producer did not already provide one.",
        ...
        "de": {
            "geoip.settings.enabled": "GeoIP-Anreicherung aktiviert",
            "geoip.settings.enabled.help": "Ergänzt bei neuen Events mit öffentlicher IP den Länder-Code, wenn der Erzeuger noch kein Land geliefert hat.",
            "geoip.settings.provider": "GeoIP-Provider",
